...
* 子オブジェクトに対する`Move`の移動量や`QRotation`の回転軸はすべて**親オブジェクトのローカル座標系**で計算されます。
* `Remove`で親オブジェクトを削除した場合、**子オブジェクト以下もすべて削除**されます。

移動量や回転軸の親オブジェクトのローカル座標系での指定は空間エディタでのオブジェクトの操作をイメージすると理解しやすいかもしれません。<br>
...
obj.ApplyTransform();
```
親の姿勢を変えた状態で子を生成した場合も、位置や回転軸は親の座標系で計算されるため、立方体が最初から回転した状態で生成されます。
```typescript
